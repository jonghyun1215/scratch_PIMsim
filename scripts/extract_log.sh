...
log_folder="$PWD/log"
output_folder="$PWD/summary"
result_file=""


# 모든 *.log 파일에 대해 반복
for logfile in "$log_folder"/*.log; do
    
    # 파일명에서 matrix base 이름 추출 (예: ASIC_100k.log -> ASIC_100k)
    matrix=$(basename "$logfile" | sed 's/\(.*\)\.log/\1/')
    result_file="$output_folder/${matrix}.txt"
...
echo "All logs processed. Summary files are in $output_folder."