...
// Thread data structure for partial overlap calculation
struct ThreadData {
...
void* countOverlapPart(void* arg) {
    ThreadData* data = static_cast<ThreadData*>(arg);
    const COOMatrix* matrix = data->matrix;
    int reference_column = data->reference_column;
    int col = data->col;

    // Local containers for rows in the two columns
    std::vector<uint32_t> rows_ref;
    std::vector<uint32_t> rows_col;

    for (size_t i = data->start; i < data->end; ++i) {
        if (matrix->col_indices[i] == reference_column) {
            rows_ref.push_back(matrix->row_indices[i]);
        }
        if (matrix->col_indices[i] == col) {
            rows_col.push_back(matrix->row_indices[i]);
        }
    }

    // Sort both vectors
    std::sort(rows_ref.begin(), rows_ref.end());
    std::sort(rows_col.begin(), rows_col.end());

    // Compute intersection
    std::vector<uint32_t> intersect;
    std::set_intersection(rows_ref.begin(), rows_ref.end(), rows_col.begin(), rows_col.end(),
                          std::back_inserter(intersect));

    data->overlap_count = intersect.size();
    return nullptr;
}

int countRowIndexOverlapCOO(const COOMatrix& matrix, int reference_column, int col, int num_threads) {
    // Partition data for threads
    std::vector<ThreadData> thread_data(num_threads);
    std::vector<pthread_t> threads(num_threads);

    size_t chunk_size = matrix.nnz / num_threads;
    for (int i = 0; i < num_threads; ++i) {
        size_t start = i * chunk_size;
        size_t end = (i == num_threads - 1) ? matrix.nnz : (i + 1) * chunk_size;

        thread_data[i] = {&matrix, reference_column, col, start, end, 0};
        pthread_create(&threads[i], nullptr, countOverlapPart, &thread_data[i]);
    }
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        total_overlap += thread_data[i].overlap_count;
    }
...
std::vector<std::vector<int>> groupColumnsCOO(const COOMatrix& matrix, const std::vector<int>& reference_columns, int num_BG) {
    ...
    do {
        ...
        int overlap = countRowIndexOverlapCOO(matrix, reference_column, col, N_THREAD);
...
int main() {
    ...
    const std::string out_file = "./generated_mtx/"+file_path; // Update with your output MTX file path
    const int num_BG = 64;
    ...
    for (size_t bg = 0; bg < all_new_row_indices.size(); ++bg) {
        std::string output_file = out_file + "-bg_" + std::to_string(bg) + ".mtx";
