    ...
    std::vector<uint32_t>& row_indices, std::vector<uint32_t>& col_indices, std::vector<uint16_t>& val, uint32_t& NEW_NNZ) 
{
    ...
    for(uint32_t i = 0; i <= t / GROUP_SIZE; i++) {
        for(uint32_t j = 0; j < GROUP_SIZE; j++) {
            ...
            store_format[i].vec[j] = 1; // Assumes vec is defined with 1
...
int main() {
    ...
    for (int tile = 0; tile < num_tiles; ++tile) {
        ...
        std::cout<< draf_result[tile].size() << std::endl;

        if(STORE)
        {
            uint32_t start_col = tile * (original_matrix.n_cols / num_tiles) + std::min(tile, (int)(original_matrix.n_cols % num_tiles));
            uint32_t end_col = start_col + (original_matrix.n_cols / num_tiles) + (tile < (original_matrix.n_cols % num_tiles) ? 1 : 0);

            std::string output_file = "./output_tiles_cpp/tile_" + std::to_string(tile) + ".mtx";

            saveCOOMatrixToMTX(output_file, tiles[tile]);
            // Compare tile with original matrix
            if (compareTileFileWithOriginal(original_matrix, output_file, start_col, end_col)) {
                std::cout << "Tile " << tile << " matches the original matrix." << std::endl;
            } else {
                std::cout << "Tile " << tile << " does not match the original matrix." << std::endl;
                return 0;
            }    
        }
    }

    std::cout << "All tiles processed" << std::endl;
    saveResultToFile(draf_result, "tiled_draf.dat");

    if(DEBUG){
        auto loadedResult = loadResultFromFile("tiled_draf.dat", 64);
    ...
    std::cout << "Saving matrix completed." << std::endl;
